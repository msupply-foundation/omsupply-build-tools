...
# Use tag instead of branch for published builds
REMOTE_SERVER_BRANCH='V1-beta-buildtest2'
...
# If entry is updated, FORCE_REBUILD_WITH_DATA should also be updated
FORCE_REBUILD_ENTRY='1'
...
if $SHOULD_BUILD_BASE; then
  ...
  docker build \
    ...
    --build-arg FORCE_REBUILD_ENTRY="$FORCE_REBUILD_ENTRY" \
    docker/build_empty/.
